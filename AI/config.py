...
def get_config(FLAGS, is_train):
  ...
  if is_train:
    ...
    config.lr = 1e-4
    config.iteration = 10e4
    
    config.ReportInterval  = 50
    config.SavingInterval  = 500

    config.image_options = {'resize': True, 'resize_size': config.im_size,  'flip': True, 'rotate_stepwise': True,"environment factor":True}
    ...
    config.image_options = {'resize': False, 'resize_size': config.im_size, 'flip': False, 'rotate_stepwise': False,"environment factor":False}
    config.fileformat = 'png'
